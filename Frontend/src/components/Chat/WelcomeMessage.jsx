...
export default function WelcomeMessage({ onSuggestionClick }) {
  ...
  return (
    <div className="flex flex-col items-center justify-start h-full text-center animate-fadeIn p-4 md:p-6 lg:p-8 overflow-y-auto">
      {/* Main Logo */}
      ...
      <div className="mb-6 md:mb-8 max-w-2xl">
        ...
        <p className="text-lg md:text-xl text-slate-300 leading-relaxed">
          Your intelligent assistant. Ask questions, get help with code, 
          research topics, or just have a conversation. I'm here to help!
        </p>
      </div>

      {/* Features Grid */}
      {/* <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8 max-w-4xl w-full">
        {features.map((feature, index) => (
          <div 
            key={index}
            className="p-3 md:p-4 bg-slate-800/30 rounded-xl border border-slate-700/50 hover:bg-slate-700/40 transition-all duration-200 hover-lift"
          >
      ...
      <div className="w-full max-w-4xl">
        <h4 className="text-base md:text-lg font-semibold text-white mb-4">Try asking me about:</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {suggestions.map((suggestion, index) => (
            <button
              key={index}
              onClick={() => onSuggestionClick(suggestion)}
              className="group p-3 md:p-4 bg-slate-800/30 hover:bg-slate-700/50 rounded-xl text-left transition-all duration-200 border border-slate-700/50 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/10 hover-lift"
            >
      ...
      {/* Bottom Stats */}
      <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-6 md:space-x-8 mt-6 md:mt-8 text-xs md:text-sm text-slate-400">
